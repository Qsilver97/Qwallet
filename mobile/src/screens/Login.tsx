...
const Login: React.FC = () => {
  ...
  return (
    <View style={tw`bg-gray-200 max-w-xs mx-auto p-10 rounded-lg shadow-2xl`}>
      <Image source={require('../../assets/images/logo.png')} style={tw`w-24 h-24 mx-auto`} />
      ...
      <View style={tw`relative mb-4`}>
        ...
        />
        <TouchableOpacity
          onPress={() => setPasswordInputType(prev => prev === 'text' ? 'password' : 'text')}
          style={tw`absolute inset-y-0 right-0 flex items-center pr-3`}
        >
          <FontAwesomeIcon icon={passwordInputType === 'password' ? faEye : faEyeSlash} size={20} />
        </TouchableOpacity>
      </View>
      <TouchableOpacity
        onPress={handleLogin}
